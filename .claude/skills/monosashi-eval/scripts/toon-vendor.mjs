...
function parseArrayHeaderLine(content, defaultDelimiter, strict = false) {
	...
	if (trimmedToken.startsWith("\"")) {
		...
		// monosashi-patch:scalar-bracket — an unquoted array-header key is a bare token (`key[N]:` / `key[N]{f}:`)
		// and cannot contain a key/value colon. If a ':' precedes the first '[', that bracket is
		// inside the scalar VALUE (e.g. `snippet: "x = []"`), not a header — so this is a scalar
		// line. Bail out instead of mis-detecting it and throwing in strict mode; the caller then
		// falls through to its `key: value` scalar path.
		if (bracketStart !== -1) {
			const keyValueColon = content.indexOf(":");
			if (keyValueColon !== -1 && keyValueColon < bracketStart) return;
		}
	}

...
# Model tiering (§8 cost): scoring is the quality-critical 0–4 rubric judgement — it inherits
# the session model (typically the strongest) and is deliberately NOT downgraded.
model: inherit
# Pack-only enforcement (§7 token cut): the S2.6-validated evidence pack is the judge's sole
# evidence source, so the whole-target scan tools (Glob/Grep) are withheld — they are what drive
# the runaway re-reads that blow up a pass (a 1-criterion tie-break should read 1 pack item, not
# re-scan the bundle). Read stays only for the rare no-pack fallback (open a specific cited path).
tools: Read, Write
---

...
# Model tiering (§8 cost): although the S2.6 gate range-validates the output, WHICH lines get cited
# (relevance/coverage of the evidence) is the dominant source of score variance downstream — so this
# stage is NOT downgraded; it inherits the session model (typically the strongest), same as judge.
model: inherit

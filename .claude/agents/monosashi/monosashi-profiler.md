...
# Model tiering (§8 cost): the axis profile drives track selection AND the mechanical M2, so a
# misjudged axis skews scores systematically — this gate judgement is NOT downgraded; it inherits
# the session model (typically the strongest), same as judge.
model: inherit

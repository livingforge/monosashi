...
# Model tiering (§8 cost): the single read produces BOTH the axis gate AND the evidence pack, and
# evidence-extraction quality is the dominant source of score variance downstream — so this stage is
# deliberately NOT downgraded; it inherits the session model (typically the strongest), same as judge.
model: inherit

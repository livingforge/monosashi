...
description: Run the full Monosashi evaluation over a folder of AI artifacts (agents / skills / harnesses / knowledge-docs) via the monosashi-eval skill. Use when the user asks to "evaluate", "score", "assess", or "評価" a directory of agents/skills/harnesses. Orchestrates inventory → read (single-read profile+evidence) → select-tracks → score (full pass A + targeted pass B) → aggregate, delegating judgement to monosashi-surveyor and monosashi-judge, and produces a radar scoreboard with the mechanical declaration-divergence (M2) flag surfaced independently. Static reading only — never executes the target.
tools: ['agent', 'read/readFile', 'search/textSearch', 'search/fileSearch', 'edit/createFile', 'edit/createDirectory', 'execute/runInTerminal']
# monosashi-surveyor + monosashi-judge drive the default single-read pipeline;
# monosashi-profiler + monosashi-gatherer are the split-mode (two-read) alternative.
agents: ['monosashi-surveyor', 'monosashi-profiler', 'monosashi-gatherer', 'monosashi-judge']
user-invocable: true
---
